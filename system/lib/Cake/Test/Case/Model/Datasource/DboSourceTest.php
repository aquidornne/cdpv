 ...
 */
	public function testMergeAssociations() {
		$data = array('Event2' => array(
				'id' => '1', 'user_id' => '1', 'title' => 'First Event',
				'body' => 'First Event Body', 'published' => 'Y',
		...
		$data = array(
			'Event2' => array(
				'id' => '1', 'user_id' => '1', 'title' => 'First Event', 'body' => 'First Event Body', 'published' => 'Y', 'created' => '2007-03-18 10:39:23', 'updated' => '2007-03-18 10:41:31'
			)
		);
 ...
 */
	public function testVirtualFieldsInOrder() {
		...
		$result = $this->db->order($order, 'ASC', $Event);
		$expected = ' ORDER BY (1 + 1) ASC, (NOW()) ASC';
		$this->assertEquals($expected, $result);

		$order = array('Event.two', 'Event.this_moment');
		$result = $this->db->order($order, 'ASC', $Event);
		$expected = ' ORDER BY (1 + 1) ASC, (NOW()) ASC';
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testFullTablePermutations() {
		$Event = ClassRegistry::init('Event');
		$result = $this->testDb->fullTableName($Event, false, false);
		...
		$Event->useTable = $Event->table = 'with spaces';
		$Event->tablePrefix = '';
		$result = $this->testDb->fullTableName($Event, true, false);
		$this->assertEquals('`with spaces`', $result);

		$this->loadFixtures('Event');
		$Event->useTable = $Event->table = 'articles';
		$Event->setDataSource('test');
		$testdb = $Event->getDataSource();
		$result = $testdb->fullTableName($Event, false, true);
		...
		$Event->setDataSource('noschema');
		$Event->schemaName = null;
		$result = $noschema->fullTableName($Event, false, true);
		$this->assertEquals('articles', $result);
 ...
 */
	public function testReadOnlyCallingQueryAssociationWhenDefined() {
		$this->loadFixtures('Event', 'User', 'EventsTag', 'Tag');
		ConnectionManager::create('test_no_queryAssociation', array(
			'datasource' => 'MockDataSource'
		));
		$Event = ClassRegistry::init('Event');
		$Event->Comment->useDbConfig = 'test_no_queryAssociation';
 ...
 */
	public function testQueryAssociationUnneededQueries() {
		...
		// case: Comment belongsTo User and Event
		$Comment->unbindModel(array(
			'hasOne' => array('Attachment')
		));
		$Comment->Event->unbindModel(array(
		...
		// case: Comment belongsTo Event, Event belongsTo User
		$Comment->unbindModel(array(
			'belongsTo' => array('User'),
			'hasOne' => array('Attachment')
		));
 ...
 */
	public function testFieldsUsingMethodCache() {
		...
		$Event = ClassRegistry::init('Event');
		$this->testDb->fields($Event, null, array('title', 'body', 'published'));
 ...
 */
	public function testFieldsCacheKeyWithDatasourceChange() {
		...
		$Event->setDataSource('firstschema');
		$ds = $Event->getDataSource();
		$ds->cacheMethods = true;
		$first = $ds->fields($Event, null, array('title', 'body', 'published'));

		$Event->setDataSource('secondschema');
		$ds = $Event->getDataSource();
		$ds->cacheMethods = true;
		$second = $ds->fields($Event, null, array('title', 'body', 'published'));
 ...
 */
	public function testFieldsCacheKeyWithSchemanameChange() {
		...
		$ds = $Event->getDataSource();
		$ds->cacheMethods = true;
		$first = $ds->fields($Event);

		$Event->schemaName = 'secondSchema';
		$ds = $Event->getDataSource();
		$ds->cacheMethods = true;
		$second = $ds->fields($Event);
	...
	public static function joinStatements() {
		return array(
			...
			array(array(
				...
				'conditions' => 'Stock.article_id = Event.id'
			), 'LEFT JOIN (SELECT Stock.article_id, sum(quantite) quantite FROM stocks AS Stock GROUP BY Stock.article_id) AS Stock ON (Stock.article_id = Event.id)')
		);
	}
 ...
 */
	public function testConditionKeysToString() {
		...
		$conditions = array('Event.name' => 'just text');
		$result = $db->conditionKeysToString($conditions, true, $Event);
		$expected = "Event.name = just text";
		...
		$conditions = array('Event.name' => array('just text', 'other text'));
		$result = $db->conditionKeysToString($conditions, true, $Event);
		$expected = "Event.name IN (just text, other text)";
		$this->assertEquals($expected, $result[0]);
	}
 ...
 */
	public function testConditionKeysToStringVirtualField() {
		...
		$conditions = array('Event.extra' => 'just text');
		$result = $db->conditionKeysToString($conditions, true, $Event);
		$expected = "(" . $Event->virtualFields['extra'] . ") = just text";
		$this->assertEquals($expected, $result[0]);
		...
		$conditions = array('Event.extra' => array('just text', 'other text'));
		$result = $db->conditionKeysToString($conditions, true, $Event);
		$expected = "(" . $Event->virtualFields['extra'] . ") IN (just text, other text)";
		$this->assertEquals($expected, $result[0]);
	}
