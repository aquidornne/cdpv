 ...
 */
	public function testFieldDoubleEscaping() {
		...
		$this->Model = $this->getMock('Event2', array('getDataSource'));
		$this->Model->alias = 'Event';
		$this->Model->expects($this->any())
			->method('getDataSource')
			->will($this->returnValue($test));

		$this->assertEquals('`Event`.`id`', $this->Model->escapeField());
		$result = $test->fields($this->Model, null, $this->Model->escapeField());
		$this->assertEquals(array('`Event`.`id`'), $result);

		$test->expects($this->at(0))->method('execute')
			->with('SELECT `Event`.`id` FROM ' . $test->fullTableName('articles') . ' AS `Event`   WHERE 1 = 1');
 ...
 */
	public function testGenerateAssociationQuerySelfJoinWithConditions() {
		...
		$this->Featured2->bindModel(array(
			'belongsTo' => array(
				'EventFeatured2' => array(
					'conditions' => 'EventFeatured2.published = \'Y\'',
		...
		$binding = array('type' => 'belongsTo', 'model' => 'EventFeatured2');
		...
		$this->assertRegExp(
			...
			'`EventFeatured2`\.`id`, `EventFeatured2`\.`title`, `EventFeatured2`\.`user_id`, `EventFeatured2`\.`published`\s+' .
			'FROM\s+\S+`featured2` AS `Featured2`\s+LEFT JOIN\s+\S+`article_featured` AS `EventFeatured2`' .
			'\s+ON\s+\(`EventFeatured2`.`published` = \'Y\'\s+AND\s+`Featured2`\.`article_featured2_id` = `EventFeatured2`\.`id`\)' .
			'\s+WHERE\s+1\s+=\s+1\s*$/',
			$result
		);
	}
 ...
 */
	public function testStringConditionsParsing() {
		...
		$result = $this->Dbo->conditions("lower(Event.title) LIKE 'a%'");
		$expected = " WHERE lower(`Event`.`title`) LIKE 'a%'";
		$this->assertEquals($expected, $result);
		...
		$result = $this->Dbo->conditions('DATEDIFF(NOW(),Event.published) < 1 && Event.live=1');
		$expected = " WHERE DATEDIFF(NOW(),`Event`.`published`) < 1 && `Event`.`live`=1";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testArrayConditionsParsing() {
		...
		$result = $this->Dbo->conditions(array('lower(Event.title)' => 'secrets'));
		$expected = " WHERE lower(`Event`.`title`) = 'secrets'";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testConditionsWithModel() {
		...
		$result = $this->Dbo->conditions(array('Event2.viewed >=' => 0), true, true, $this->Model);
		$expected = " WHERE `Event2`.`viewed` >= 0";
		$this->assertEquals($expected, $result);
		...
		$result = $this->Dbo->conditions(array('Event2.viewed >=' => '1'), true, true, $this->Model);
		$expected = " WHERE `Event2`.`viewed` >= 1";
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->conditions(array('Event2.rate_sum BETWEEN ? AND ?' => array(0, 10)), true, true, $this->Model);
		$expected = " WHERE `Event2`.`rate_sum` BETWEEN 0 AND 10";
		$this->assertEquals($expected, $result);
		...
		$result = $this->Dbo->conditions(array('Event2.rate_sum BETWEEN ? AND ?' => array('1', '10')), true, true, $this->Model);
		$expected = " WHERE `Event2`.`rate_sum` BETWEEN 1 AND 10";
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testVirtualFields() {
		...
		$expected = array(
			...
			'`Event`.`created`',
			'`Event`.`updated`',
			'(NOW()) AS  `Event__this_moment`',
			'(1 + 1) AS  `Event__two`',
			"(SELECT COUNT(*) FROM $commentsTable WHERE `Event`.`id` = `$commentsTable`.`article_id`) AS  `Event__comment_count`"
		);

		$this->assertEquals($expected, $result);

		$result = $this->Dbo->fields($Event, null, array('this_moment', 'title'));
		$expected = array(
			'`Event`.`title`',
			'(NOW()) AS  `Event__this_moment`',
		);
		...
		$result = $this->Dbo->fields($Event, null, array('Event.title', 'Event.this_moment'));
		$expected = array(
			'`Event`.`title`',
			'(NOW()) AS  `Event__this_moment`',
		);
		...
		$result = $this->Dbo->fields($Event, null, array('Event.this_moment', 'Event.title'));
		$expected = array(
			'`Event`.`title`',
			'(NOW()) AS  `Event__this_moment`',
		);
		...
		$result = $this->Dbo->fields($Event, null, array('Event.*'));
		$expected = array(
			'`Event`.*',
			'(NOW()) AS  `Event__this_moment`',
			'(1 + 1) AS  `Event__two`',
		...
		$result = $this->Dbo->fields($Event, null, array('*'));
		$expected = array(
			'*',
			'(NOW()) AS  `Event__this_moment`',
			'(1 + 1) AS  `Event__two`',
 ...
 */
	public function testVirtualFieldsInConditions() {
		...
		$result = $this->Dbo->conditions($conditions, true, false, $Event);
		$expected = '(1 + 1) = 2';
		$this->assertEquals($expected, $result);
		...
		$expected = "(SELECT COUNT(*) FROM $commentsTable WHERE `Event`.`id` = `$commentsTable`.`article_id`) > 5";
		$result = $this->Dbo->conditions($conditions, true, false, $Event);
		$this->assertEquals($expected, $result);
 ...
 */
	public function testConditionsWithComplexVirtualFields() {
		$Event = ClassRegistry::init('Event', 'Comment', 'Tag');
		$Event->virtualFields = array(
			'distance' => 'ACOS(SIN(20 * PI() / 180)
					* SIN(Event.latitude * PI() / 180)
					+ COS(20 * PI() / 180)
					* COS(Event.latitude * PI() / 180)
					* COS((50 - Event.longitude) * PI() / 180)
				) * 180 / PI() * 60 * 1.1515 * 1.609344'
		);
		$conditions = array('distance >=' => 20);
		$result = $this->Dbo->conditions($conditions, true, true, $Event);

		$this->assertRegExp('/\) >= 20/', $result);
		$this->assertRegExp('/[`\'"]Event[`\'"].[`\'"]latitude[`\'"]/', $result);
		$this->assertRegExp('/[`\'"]Event[`\'"].[`\'"]longitude[`\'"]/', $result);
	}
 ...
 */
	public function testVirtualFieldsInCalculate() {
		...
		$result = $this->Dbo->calculate($Event, 'count', array('this_moment'));
		$expected = 'COUNT(NOW()) AS `count`';
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->calculate($Event, 'max', array('comment_count'));
		$expected = "MAX(SELECT COUNT(*) FROM $commentsTable WHERE `Event`.`id` = `$commentsTable`.`article_id`) AS `comment_count`";
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testReadVirtualFieldsWithNewLines() {
		...
		$result = $this->Dbo->fields($Event, null, array());
		$result = $this->Dbo->fields($Event, $Event->alias, $result);
 ...
 */
	public function testVirtualFieldsInGroup() {
		$Event = ClassRegistry::init('Event');
		$Event->virtualFields = array(
			'this_year' => 'YEAR(Event.created)'
		);

		$result = $this->Dbo->group('this_year', $Event);

		$expected = " GROUP BY (YEAR(`Event`.`created`))";
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testFieldsWithComplexVirtualFields() {
		...
		$result = $this->Dbo->fields($Event, null, $fields);
		$qs = $this->Dbo->startQuote;
		$qe = $this->Dbo->endQuote;

		$this->assertEquals("{$qs}Event{$qe}.{$qs}id{$qe}", $result[0]);
		$this->assertRegExp('/Event__distance/', $result[1]);
		$this->assertRegExp('/[`\'"]Event[`\'"].[`\'"]latitude[`\'"]/', $result[1]);
		$this->assertRegExp('/[`\'"]Event[`\'"].[`\'"]longitude[`\'"]/', $result[1]);
	}
	...
	public function testVirtualFieldsFetch() {
		...
		$Event->virtualFields = array(
			'comment_count' => 'SELECT COUNT(*) FROM ' . $this->Dbo->fullTableName('comments') .
				' WHERE Event.id = ' . $this->Dbo->fullTableName('comments') . '.article_id'
		);

		$conditions = array('comment_count >' => 2);
		$query = 'SELECT ' . implode(',', $this->Dbo->fields($Event, null, array('id', 'comment_count'))) .
				' FROM ' . $this->Dbo->fullTableName($Event) . ' Event ' . $this->Dbo->conditions($conditions, true, true, $Event);
		$result = $this->Dbo->fetchAll($query);
		$expected = array(array(
			'Event' => array('id' => 1, 'comment_count' => 4)
		));
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testVirtualFieldsComplexRead() {
		$this->loadFixtures('DataTest', 'Event', 'Comment', 'User', 'Tag', 'EventsTag');
		...
		$Event->virtualFields = array(
			'comment_count' => 'SELECT COUNT(*) FROM ' . $commentTable .
				' AS Comment WHERE Event.id = Comment.article_id'
		);
		$result = $Event->find('all');
		$this->assertTrue(count($result) > 0);
		$this->assertTrue($result[0]['Event']['comment_count'] > 0);
 ...
 */
	public function testRealQueries() {
		$this->loadFixtures('Apple', 'Event', 'User', 'Comment', 'Tag', 'Sample', 'EventsTag');
		...
		$result = $this->Dbo->read($Event, array(
 ...
 */
	public function testUpdateStatements() {
		...
		$this->Dbo->expects($this->at(1))->method('execute')
			->with("UPDATE `$db`.`articles` AS `Event` LEFT JOIN `$db`.`users` AS `User` ON " .
				"(`Event`.`user_id` = `User`.`id`)" .
				" SET `Event`.`field1` = 2  WHERE 2=2");

		$this->Dbo->expects($this->at(2))->method('execute')
			->with("UPDATE `$db`.`articles` AS `Event` LEFT JOIN `$db`.`users` AS `User` ON " .
				"(`Event`.`user_id` = `User`.`id`)" .
				" SET `Event`.`field1` = 'value'  WHERE `index` = 'val'");

		$Event = new Event();

		$this->Dbo->update($Event, array('field1'), array('value1'));
		$this->Dbo->update($Event, array('field1'), array('2'), '2=2');
		$this->Dbo->update($Event, array('field1'), array("'value'"), array('index' => 'val'));
	}
 ...
 */
	public function testDeleteStatements() {
		...
		$this->Dbo->expects($this->at(1))->method('execute')
			->with("DELETE `Event` FROM `$db`.`articles` AS `Event` LEFT JOIN `$db`.`users` AS `User` " .
		...
		$this->Dbo->delete($Event);
		$this->Dbo->delete($Event, true);
		$this->Dbo->delete($Event, '2=2');
	}
