 ...
 */
	public function testVirtualFields() {
		$this->loadFixtures('Event', 'Comment', 'User', 'Attachment', 'Tag', 'EventsTag');
		$Event = new Event;
		$Event->virtualFields = array(
			'next_id' => 'Event.id + 1',
			'complex' => 'Event.title || Event.body',
			'functional' => 'COALESCE(User.user, Event.title)',
			'subquery' => 'SELECT count(*) FROM ' . $Event->Comment->table
		);
		$result = $Event->find('first');
		$this->assertEquals(2, $result['Event']['next_id']);
		$this->assertEquals($result['Event']['complex'], $result['Event']['title'] . $result['Event']['body']);
		$this->assertEquals($result['Event']['functional'], $result['User']['user']);
		$this->assertEquals(6, $result['Event']['subquery']);
	}
 ...
 */
	public function testVirtualFieldAsAConstant() {
		...
		$result = $Event->find('first');
		$this->assertNull($result['Event']['empty']);
		$this->assertTrue($result['Event']['truth']);
		$this->assertEquals(43, $result['Event']['number']);
	}
 ...
 */
	public function testQuoteDistinctInFunction() {
		...
		$Event = new Event;
		$result = $this->Dbo->fields($Event, null, array('COUNT(DISTINCT Event.id)'));
		$expected = array('COUNT(DISTINCT "Event"."id")');
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->fields($Event, null, array('COUNT(DISTINCT id)'));
		$expected = array('COUNT(DISTINCT "id")');
		$this->assertEquals($expected, $result);

		$result = $this->Dbo->fields($Event, null, array('COUNT(DISTINCT FUNC(id))'));
 ...
 */
	public function testUpdateAllWithNonQualifiedConditions() {
		$this->loadFixtures('Event');
		$Event = new Event();
		$result = $Event->updateAll(array('title' => "'Awesome'"), array('title' => 'Third Event'));
		$this->assertTrue($result);

		$result = $Event->find('count', array(
			'conditions' => array('Event.title' => 'Awesome')
		));
		$this->assertEquals(1, $result, 'Event count is wrong or fixture has changed.');
	}
