 ...
 */
	public function testOldQuery() {
		$this->loadFixtures('Event', 'User', 'Tag', 'EventsTag', 'Comment', 'Attachment');
		$Event = new Event();
		...
		$results = $Event->query($query, false);
		...
		$results = $Event->query($query, true);
 ...
 */
	public function testPreparedQuery() {
		$this->loadFixtures('Event', 'User', 'Tag', 'EventsTag');
		$Event = new Event();
		...
		$params = array(1, 'Y');
		$result = $Event->query($query, $params);
		$expected = array(
			'0' => array(
				$this->db->fullTableName('articles', false, false) => array(
					'title' => 'First Event', 'published' => 'Y')
		));
		...
		$params = array('First Event');
		$result = $Event->query($query, $params, false);
		...
		$params = array('First? Event', 'Y');
		$Event->query($query, $params);
 ...
 */
	public function testFindNeighbors() {
		$this->loadFixtures('User', 'Event', 'Comment', 'Tag', 'EventsTag', 'Attachment');
		$TestModel = new Event();
		...
		$this->assertEquals(array('id' => 2), $result['next']['Event']);
		...
		$this->assertEquals(array('id' => 2), $result['prev']['Event']);
 ...
 */
	public function testFindNeighborsNoPrev() {
		...
		$result = $Event->find('neighbors', array(
			'field' => 'Event.title',
			'value' => 'Second Event',
			'fields' => array('id'),
			'conditions' => array(
				'Event.title LIKE' => '%Event%'
			),
			'recursive' => 0,
		));
		$expected = array(
			...
			array(
				...
				'Comment' => array(
					array(
						...
						'user_id' => '4',
						'comment' =>
						'Second Comment for First Event',
						'published' => 'Y',
						'created' => '2007-03-18 10:47:23',
						'updated' => '2007-03-18 10:49:31'
		))));
 ...
 */
	public function testBindMultipleTimes() {
		$this->loadFixtures('User', 'Comment', 'Event', 'Tag', 'EventsTag');
		$TestModel = new User();
 ...
 */
	public function testBindModelMultipleTimesResetCorrectly() {
		$this->loadFixtures('User', 'Comment', 'Event');
		$TestModel = new User();
 ...
 */
	public function testDeeperAssociationAfterFind() {
		$this->loadFixtures('Post', 'Author', 'Comment', 'Attachment', 'Event');
 ...
 */
	public function testHabtmFinderQuery() {
		...
		$sql = $this->db->buildStatement(
			array(
				'fields' => $this->db->fields($Event->Tag, null, array(
					'Tag.id', 'Tag.tag', 'EventsTag.article_id', 'EventsTag.tag_id'
				)),
				...
				'joins' => array(array(
					...
					'table' => 'articles_tags',
					'conditions' => array(
						array("EventsTag.article_id" => '{$__cakeID__$}'),
						array("EventsTag.tag_id" => $this->db->identifier('Tag.id'))
					)
				)),
				'conditions' => array(),
				'order' => null
		...
		$Event->hasAndBelongsToMany['Tag']['finderQuery'] = $sql;
		$result = $Event->find('first');
		$expected = array(
			array(
				'id' => '1',
				'tag' => 'tag1'
 ...
 */
	public function testFindAllRecursiveSelfJoin() {
		$this->loadFixtures('Home', 'AnotherEvent', 'Advertisement');
		$TestModel = new Home();
		$TestModel->recursive = 2;
 ...
 */
	public function testConditionalNumerics() {
		$this->loadFixtures('NumericEvent');
		$NumericEvent = new NumericEvent();
		$data = array('conditions' => array('title' => '12345abcde'));
		$result = $NumericEvent->find('first', $data);
		$this->assertTrue(!empty($result));
 ...
 */
	public function testGenerateFindList() {
		$this->loadFixtures('Event', 'Apple', 'Post', 'Author', 'User', 'Comment');
		...
		if ($db instanceof Mysql) {
			$result = $TestModel->find('list', array(
				'order' => array('FIELD(Event.id, 3, 2) ASC', 'Event.title ASC')
			));
			$expected = array(
				1 => 'First Event',
				3 => 'Third Event',
		...
		$result = Hash::combine(
			...
			'{n}.Event.id', '{n}.Event', '{n}.Event.user_id'
		);
		$expected = array(
			1 => array(
				1 => array(
		...
		$result = Hash::combine(
			...
			'{n}.Event.id', '{n}.Event.title', '{n}.Event.user_id'
		);
 ...
 */
	public function testFindCount() {
		$this->loadFixtures('User', 'Event', 'Comment', 'Tag', 'EventsTag');
		...
		$expected = count($Event->find('all', array(
			'fields' => array('Event.user_id'),
			'group' => 'Event.user_id')
		));
		$result = $Event->find('count', array('group' => array('Event.user_id')));
		$this->assertEquals($expected, $result);

		$expected = count($Event->find('all', array(
			'fields' => array('Event.user_id'),
			'conditions' => array('Event.user_id' => 1),
			'group' => 'Event.user_id')
		));
		$result = $Event->find('count', array(
			'conditions' => array('Event.user_id' => 1),
			'group' => array('Event.user_id'),
		));
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testRead() {
		...
		$result = $TestModel->bindModel(array('hasMany' => array('Event')));
		$this->assertTrue($result);
 ...
 */
	public function testRecursiveRead() {
		...
		$result = $TestModel->bindModel(array('hasMany' => array('Event')), false);
		$this->assertTrue($result);
	...
	public function testRecursiveFindAll() {
		...
		$result = $TestModel->find('all', array('conditions' => array('Event.user_id' => 1)));
		$expected = array(
			array(
				'Event' => array(
					'id' => '1',
		...
		$result = $TestModel->find('all', array(
			'conditions' => array('Event.user_id' => 3),
			'limit' => 1,
			'recursive' => 2
		));
		...
		$Featured->bindModel(array(
			'belongsTo' => array(
				'EventFeatured' => array(
					'conditions' => "EventFeatured.published = 'Y'",
					'fields' => 'id, title, user_id, published'
				)
			)
		));

		$Featured->EventFeatured->unbindModel(array(
			'hasMany' => array('Attachment', 'Comment'),
			'hasAndBelongsToMany' => array('Tag'))
		);

		$orderBy = 'EventFeatured.id ASC';
 ...
 */
	public function testfindCustom() {
		...
		$data = array('user_id' => 3, 'title' => 'Fourth Event', 'body' => 'Event Body, unpublished', 'published' => 'N');
		$Event->create($data);
		$Event->save(null, false);
		$this->assertEquals(4, $Event->id);

		$result = $Event->find('published');
		$this->assertEquals(3, count($result));

		$result = $Event->find('unPublished');
