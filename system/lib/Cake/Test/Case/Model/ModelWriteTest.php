 ...
 */
	public function testSaveDateAsFirstEntry() {
		$this->loadFixtures('Event', 'User', 'Comment', 'Attachment', 'Tag', 'EventsTag');
		...
		$testResult = $Event->find('first', array('conditions' => array('Event.title' => 'Test Title')));

		$this->assertEquals($data['Event']['title'], $testResult['Event']['title']);
		$this->assertEquals('2008-01-01 00:00:00', $testResult['Event']['created']);
	}
 ...
 */
	public function testSaveField() {
		...
		$TestModel->id = 1;
		$result = $TestModel->saveField('title', 'New First Event');
		$this->assertFalse(empty($result));
		...
		$result['Event']['title'] = trim($result['Event']['title']);
		$this->assertEquals($expected, $result);

		$TestModel->id = 1;
		$TestModel->set('body', 'Messed up data');
		$result = $TestModel->saveField('title', 'First Event');
		$this->assertFalse(empty($result));
		$result = $TestModel->read(array('id', 'user_id', 'title', 'body'), 1);
		$expected = array('Event' => array(
			'id' => '1',
 ...
 */
	public function testSaveWithSet() {
		...
		$data = array('Event' => array('id' => '5', 'title' => 'Fifth Event - New Title 5'));
		$result = ($TestModel->set($data) && $TestModel->save());
		$this->assertFalse(empty($result));
		...
		$expected = array(
			array('Event' => array('id' => 1, 'title' => 'First Event')),
			array('Event' => array('id' => 2, 'title' => 'Second Event')),
			array('Event' => array('id' => 3, 'title' => 'Third Event')),
			array('Event' => array('id' => 4, 'title' => 'Fourth Event - New Title')),
			array('Event' => array('id' => 5, 'title' => 'Fifth Event - New Title 5'))
		);
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testSaveFromXml() {
		...
		$Event = new Event();
		$result = $Event->save(Xml::build('<article title="test xml" user_id="5" />'));
		$this->assertFalse(empty($result));
		$results = $Event->find('first', array('conditions' => array('Event.title' => 'test xml')));
		$this->assertFalse(empty($results));

		$result = $Event->save(Xml::build('<article><title>testing</title><user_id>6</user_id></article>'));
		$this->assertFalse(empty($result));
		$results = $Event->find('first', array('conditions' => array('Event.title' => 'testing')));
		$this->assertFalse(empty($results));

		$result = $Event->save(Xml::build('<article><title>testing with DOMDocument</title><user_id>7</user_id></article>', array('return' => 'domdocument')));
		$this->assertFalse(empty($result));
		$results = $Event->find('first', array('conditions' => array('Event.title' => 'testing with DOMDocument')));
		$this->assertFalse(empty($results));
	}
 ...
 */
	public function testSaveHabtm() {
		$this->loadFixtures('Event', 'User', 'Comment', 'Tag', 'EventsTag');
		$TestModel = new Event();
		...
		$expected = array(
			...
			'Comment' => array(
				array(
					...
					'article_id' => '2',
					'user_id' => '1',
					'comment' => 'First Comment for Second Event',
					'published' => 'Y',
				...
				array(
					...
					'article_id' => '2',
					'user_id' => '2',
					'comment' => 'Second Comment for Second Event',
					'published' => 'Y',
		...
		$result = $TestModel->find('first', array('fields' => array('id', 'user_id', 'title', 'body'), 'conditions' => array('Event.id' => 2)));
		$expected = array(
			'Event' => array(
				'id' => '2',
				'user_id' => '3',
		...
		$data = array('Event' => array('id' => '2'), 'Tag' => array('Tag' => array(2, 3)));
		$result = $TestModel->set($data);
		$this->assertFalse(empty($result));
		...
		$data = array(
			'Event' => array(
				'id' => 10,
				'user_id' => '2',
				'title' => 'New Event With Tags and fieldList',
				'body' => 'New Event Body with Tags and fieldList',
				'created' => '2007-03-18 14:55:23',
				'updated' => '2007-03-18 14:57:31'
			),
			'Tag' => array(
 ...
 */
	public function testCreateWithPKFiltering() {
		...
		$result = $TestModel->create(array('Event' => $data), true);
		$expected = array(
			'Event' => array(
				'published' => 'N',
				'id' => false,
		...
		$result = $TestModel->create(array(
			'Event' => array_diff_key($data, array(
				'created' => true,
				'updated' => true,
				'modified' => true
		))), true);
 ...
 */
	public function testCreationWithMultipleData() {
		$this->loadFixtures('Event', 'Comment');
		$Event = new Event();
		$Comment = new Comment();

		$articles = $Event->find('all', array(
			'fields' => array('id', 'title'),
			'recursive' => -1,
			'order' => array('Event.id' => 'ASC')
		));
		...
		$expected = array(
			array('Comment' => array(
				...
				'article_id' => 1,
				'user_id' => 2,
				'comment' => 'First Comment for First Event',
				'published' => 'Y'
			)),
			array('Comment' => array(
				'id' => 2,
				'article_id' => 1,
				'user_id' => 4,
				'comment' => 'Second Comment for First Event',
				'published' => 'Y'
			)),
			array('Comment' => array(
				'id' => 3,
				'article_id' => 1,
				'user_id' => 1,
				'comment' => 'Third Comment for First Event',
				'published' => 'Y'
			)),
			array('Comment' => array(
				'id' => 4,
				'article_id' => 1,
				'user_id' => 1,
				'comment' => 'Fourth Comment for First Event',
				'published' => 'N'
			)),
			array('Comment' => array(
				'id' => 5,
		...
		$expected = array(
			...
			array('Comment' => array(
				...
				'article_id' => 2,
				'user_id' => 2, 'comment' =>
				'Second Comment for Second Event',
				'published' => 'Y'
			)),
			array('Comment' => array(
				'id' => 7,
 ...
 */
	public function testCreationWithMultipleDataSameModel() {
		...
		$result = $Event->field('title', array('id' => 1));
		...
		$data = array(
			'Event' => array(
				...
				'body' => 'Brand New Event Body',
				'published' => 'Y'
			),
			'SecondaryEvent' => array(
				'id' => 1
		));
 ...
 */
	public function testUpdateExisting() {
		$this->loadFixtures('User', 'Event', 'Comment');
		$TestModel = new User();
		$TestModel->create();
 ...
 */
	public function testUpdateSavingBlankValues() {
		...
		$Event->create();
		$result = $Event->save(array(
			'id' => 1,
			'title' => '',
			'body' => ''
		));
		$this->assertTrue((bool)$result);
		$result = $Event->find('first', array('conditions' => array('Event.id' => 1)));
		$this->assertEquals('', $result['Event']['title'], 'Title is not blank');
		$this->assertEquals('', $result['Event']['body'], 'Body is not blank');
	}
 ...
 */
	public function testUpdateMultiple() {
		$this->loadFixtures('Comment', 'Event', 'User', 'CategoryThread');
 ...
 */
	public function testSaveAll() {
		$this->loadFixtures('Post', 'Author', 'Comment', 'Attachment', 'Event', 'User');
		$TestModel = new Post();
 ...
 */
	public function testSaveAllHabtm() {
		$this->loadFixtures('Event', 'Tag', 'Comment', 'User', 'EventsTag');
		$data = array(
			'Event' => array(
				'user_id' => 1,
				'title' => 'Event Has and belongs to Many Tags'
			),
			'Tag' => array(
				'Tag' => array(1, 2)
			),
 ...
 */
	public function testSaveAllBelongsTo() {
		...
		$model->Event->deleteAll(true);
		$this->assertEquals(array(), $model->Event->find('all'));
		...
		$result = $model->find('all', array('fields' => array(
			'Comment.id', 'Comment.comment', 'Comment.article_id', 'Event.id', 'Event.title'
		)));
		$expected = array(array(
			'Comment' => array(
				'id' => '1',
 ...
 */
	public function testSaveAllAtomic() {
		$this->loadFixtures('Event', 'User', 'Comment');
		$TestModel = new Event();
		...
		$this->assertSame($result, array('Event' => true, 'Comment' => array(true)));
		...
		$this->assertSame($result, array('Event' => true, 'Comment' => array(true, true)));
 ...
 */
	public function testSaveAllDeepAssociated() {
		$this->loadFixtures('Event', 'Comment', 'User', 'Attachment');
		$TestModel = new Event();
		...
		$result = $TestModel->findById(2);
		$expected = array(
			'First Comment for Second Event',
			'Second Comment for Second Event',
			'First new comment',
			'Second new comment'
		);
		...
		$data = array(
			...
			'Comment' => array(
				...
				'user_id' => 5,
				'Event' => array(
					'title' => 'First Event deepsave insert',
					'body' => 'First Event Body deepsave insert',
					'User' => array(
						'user' => '',
						'password' => 'magic'
					),
		...
		$data['Comment']['Event']['User']['user'] = 'deepsave';
		$TestModel->Comment->Attachment->create();
		$result = $TestModel->Comment->Attachment->saveAll($data, array('deep' => true));
		$this->assertTrue($result);
		...
		unset(
			$result['Event']['created'], $result['Event']['updated'],
 ...
 */
	public function testSaveAllNotDeepMany() {
		...
		$data = array(
			array(
				'Event' => array('id' => 1, 'body' => 'Ignore invalid comment'),
				'Comment' => array(
					array('comment' => '', 'published' => 'Y', 'user_id' => 2)
				)
			),
 ...
 */
	public function testSaveAllNotDeepValidateOnly() {
		...
		$result = $TestModel->saveAll(
			array(
				'Event' => array('id' => 2, 'body' => 'Ignore invalid user data'),
				'Comment' => array(
					array('comment' => 'First new comment', 'published' => 'Y', 'User' => array('user' => '', 'password' => 'newuserpass')),
					array('comment' => 'Second new comment', 'published' => 'Y', 'user_id' => 2)
				)
		...
		$result = $TestModel->saveAll(array(
			'Event' => array('id' => 2, 'body' => 'Ignore invalid attachment data'),
			'Comment' => array(
				array('comment' => 'Third new comment', 'published' => 'Y', 'user_id' => 5),
				array('comment' => 'Fourth new comment', 'published' => 'Y', 'user_id' => 2, 'Attachment' => array('attachment' => ''))
			)
		...
		$data = array(
			...
			'Comment' => array(
				...
				'user_id' => 5,
				'Event' => array(
					'title' => 'First Event deepsave insert ignored',
					'body' => 'First Event Body deepsave insert',
					'User' => array(
						'user' => '',
						'password' => 'magic'
 ...
 */
	public function testSaveAllHasMany() {
		...
		$this->assertEquals(array('Event' => false), $result);
 ...
 */
	public function testSaveAllValidateFirst() {
		$this->loadFixtures('Event', 'Comment', 'Attachment', 'User', 'EventsTag', 'Tag');
		$model = new Event();
		$model->deleteAll(true);
 ...
 */
	public function testSaveAllHasManyValidationOnly() {
		$this->loadFixtures('Event', 'Comment', 'Attachment');
		$TestModel = new Event();
		$TestModel->belongsTo = $TestModel->hasAndBelongsToMany = array();
		$TestModel->Comment->validate = array('comment' => 'notEmpty');
 ...
 */
	public function testSaveAllEmptyData() {
		...
		$this->loadFixtures('Event', 'ProductUpdateAll', 'Comment', 'Attachment');
		$model = new Event();
 ...
 */
	public function testSaveAssociatedAtomicFalseValidateFirstWithErrors() {
		$this->loadFixtures('Comment', 'Event', 'User');
		$Event = ClassRegistry::init('Event');
		$Event->Comment->validator()->add('comment', array(
			array('rule' => 'notEmpty')
		));
		...
		$Event->saveAssociated(
			$data,
			array('validate' => 'first', 'atomic' => false)
		);

		$result = $Event->validationErrors;
		$expected = array(
			'Comment' => array(
				array(
					'comment' => array('This field cannot be left blank')
 ...
 */
	public function testSaveAssociatedHabtm() {
		...
		$Event = new Event();
		$result = $Event->saveAssociated($data);
		$this->assertFalse(empty($result));
 ...
 */
	public function testSaveAssociatedHasManyEmpty() {
		...
		$result = $TestModel->saveAssociated(array(
			'Event' => array('title' => 'title', 'user_id' => 1),
			'Comment' => array()
		), array('validate' => true));
		$this->assertTrue($result);
		...
		$this->assertEquals(array('Event' => true), $result);

		//empty primary data is not ignored
		$result = $TestModel->saveAssociated(array('Event' => array()), array('validate' => true));
		$this->assertFalse($result);

		$result = $TestModel->saveAssociated(array('Event' => array()), array('validate' => true, 'atomic' => false));
		$this->assertEquals(array('Event' => false), $result);
	}
 ...
 */
	public function testValidateAssociated() {
		$this->loadFixtures('Attachment', 'Event', 'Comment');
		$TestModel = new Comment();
		$TestModel->Attachment->validate = array('attachment' => 'notEmpty');
 ...
 */
	public function testPkInReturnArrayForCreate() {
		...
		$result = $TestModel->save($data);
		$this->assertSame($result['Event']['id'], $TestModel->id);
	}
 ...
 */
	public function testSaveAllFieldListHasMany() {
		...
		$data = array(
			'Event' => array('title' => 'I will not save'),
			'Comment' => array(
				array('comment' => 'First new comment', 'published' => 'Y', 'user_id' => 1),
				array('comment' => 'Second new comment', 'published' => 'Y', 'user_id' => 2)
			)
		...
		$result = $TestModel->find('all');
		$this->assertEquals('', $result[0]['Event']['title']);
		...
		$this->assertEquals('', $result[1]['Event']['title']);
		...
		$this->assertEquals('', $result[2]['Event']['title']);
 ...
 */
	public function testSaveAllFieldListHasOne() {
		$this->loadFixtures('Attachment', 'Comment', 'Event', 'User');
		$TestModel = new Comment();
 ...
 */
	public function testSaveAllFieldListHasOneAddFkToWhitelist() {
		$this->loadFixtures('EventFeatured', 'Featured');
		$Event = new EventFeatured();
		$Event->belongsTo = $Event->hasMany = array();
		$Event->Featured->validate = array('end_date' => 'notEmpty');
		...
		$result = $Event->saveAll($record, array('validate' => 'only'));
		$this->assertFalse($result);
		$expected = array(
			'body' => array(
				'This field cannot be left blank'
		...
		$this->assertEquals($expected, $Event->validationErrors);

		$fieldList = array(
			'EventFeatured' => array('user_id', 'title'),
			'Featured' => array('category_id')
		);

		$result = $Event->saveAll($record, array(
			'fieldList' => $fieldList, 'validate' => 'first'
		));
		$this->assertTrue($result);
		$this->assertEmpty($Event->validationErrors);

		$Event->recursive = 0;
		$result = $Event->find('first', array('order' => array('EventFeatured.created' => 'DESC')));
		$this->assertSame($result['EventFeatured']['id'], $result['Featured']['article_featured_id']);
	}
 ...
 */
	public function testSaveAllDeepFieldListHasMany() {
		$this->loadFixtures('Event', 'Comment', 'User');
		$TestModel = new Event();
		$TestModel->belongsTo = $TestModel->hasAndBelongsToMany = array();
		...
		$result = $TestModel->saveAll(array(
			'Event' => array('id' => 2, 'title' => 'I will not save'),
			'Comment' => array(
				array('comment' => 'First new comment', 'published' => 'Y', 'user_id' => 1),
				array(
 ...
 */
	public function testSaveAllDeepHasManyBelongsTo() {
		...
		$result = $TestModel->saveAll(array(
			'Event' => array('id' => 2, 'title' => 'The title'),
			'Comment' => array(
				array('comment' => 'First new comment', 'published' => 'Y', 'user_id' => 1),
				array(
 ...
 */
	public function testSaveAllDeepOrderHasManyHasMany() {
		...
		$result = $TestModel->saveAll(array(
			'Event' => array('id' => 2, 'title' => 'Comment has its data after Attachment'),
			'Comment' => array(
				array(
					'Attachment' => array(
 ...
 */
	public function testSaveAllDeepEmptyHasManyHasMany() {
		...
		$result = $TestModel->saveAll(array(
			'Event' => array('id' => 3, 'user_id' => 1, 'title' => 'Comment has no data'),
			'Comment' => array(
				array(
					'user_id' => 1,
					'Attachment' => array(
