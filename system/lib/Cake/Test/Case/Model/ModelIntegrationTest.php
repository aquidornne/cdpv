 ...
 */
	public function testAssociationLazyLoading() {
		$this->loadFixtures('EventFeaturedsTags');
		$Event = new EventFeatured();
		$this->assertTrue(isset($Event->belongsTo['User']));
		$this->assertFalse(property_exists($Event, 'User'));
		$this->assertInstanceOf('User', $Event->User);

		$this->assertTrue(isset($Event->belongsTo['Category']));
		$this->assertFalse(property_exists($Event, 'Category'));
		$this->assertTrue(isset($Event->Category));
		$this->assertInstanceOf('Category', $Event->Category);

		$this->assertTrue(isset($Event->hasMany['Comment']));
		$this->assertFalse(property_exists($Event, 'Comment'));
		$this->assertTrue(isset($Event->Comment));
		$this->assertInstanceOf('Comment', $Event->Comment);

		$this->assertTrue(isset($Event->hasAndBelongsToMany['Tag']));
		...
		$this->assertTrue(property_exists($Event, 'Tag'));
		$this->assertTrue(isset($Event->Tag));
		$this->assertInstanceOf('Tag', $Event->Tag);

		$this->assertFalse(property_exists($Event, 'EventFeaturedsTag'));
		$this->assertInstanceOf('AppModel', $Event->EventFeaturedsTag);
		$this->assertEquals('article_featureds_tags', $Event->hasAndBelongsToMany['Tag']['joinTable']);
		$this->assertEquals('tag_id', $Event->hasAndBelongsToMany['Tag']['associationForeignKey']);
	}
 ...
 */
	public function testAssociationLazyLoadWithHABTM() {
		$this->loadFixtures('FruitsUuidTag', 'EventsTag');
		$this->db->cacheSources = false;
		$Event = new EventB();
		$this->assertTrue(isset($Event->hasAndBelongsToMany['TagB']));
		$this->assertFalse(property_exists($Event, 'TagB'));
		$this->assertInstanceOf('TagB', $Event->TagB);

		$this->assertFalse(property_exists($Event, 'EventsTag'));
		$this->assertInstanceOf('AppModel', $Event->EventsTag);
 ...
 */
	public function testAssociationLazyLoadWithBindModel() {
		...
		$this->assertFalse(isset($Event->belongsTo['User']));
		$this->assertFalse(property_exists($Event, 'User'));

		$Event->bindModel(array('belongsTo' => array('User')));
		$this->assertTrue(isset($Event->belongsTo['User']));
		$this->assertFalse(property_exists($Event, 'User'));
		$this->assertInstanceOf('User', $Event->User);
	}
 ...
 */
	public function testMissingTable() {
		$Event = new EventB(false, uniqid());
		$Event->schema();
	}

/**
 * testPkInHAbtmLinkModelEventB
 *
 * @return void
 */
	public function testPkInHabtmLinkModelEventB() {
		$this->loadFixtures('Event', 'Tag', 'EventsTag');
		$TestModel = new EventB();
		$this->assertEquals('article_id', $TestModel->EventsTag->primaryKey);
	}
 ...
 */
	public function testFindWithJoinsOption() {
		...
		$options = array(
			...
			'group' => array('User.user', 'Event.published'),
			'recursive' => -1,
			'order' => array('User.user')
		);
		$result = $TestUser->find('all', $options);
		$expected = array(
			array('User' => array('user' => 'garrett'), 'Event' => array('published' => '')),
			array('User' => array('user' => 'larry'), 'Event' => array('published' => 'Y')),
			array('User' => array('user' => 'mariano'), 'Event' => array('published' => 'Y')),
			array('User' => array('user' => 'nate'), 'Event' => array('published' => ''))
		);
 ...
 */
	public function testCrossDatabaseJoins() {
		...
		$this->loadFixtures('Event', 'Tag', 'EventsTag', 'User', 'Comment');
		$TestModel = new Event();
 ...
 */
	public function testResetOfExistsOnCreate() {
		...
		$Event = new Event();
		$Event->id = 1;
		$Event->saveField('title', 'Reset me');
		$Event->delete();
		$Event->id = 1;
		...
		$this->assertFalse($Event->exists());
		$Event->id = 2;
		$Event->saveField('title', 'Staying alive');
		$result = $Event->read(null, 2);
		$this->assertEquals('Staying alive', $result['Event']['title']);
	}
 ...
 */
	public function testPluginAssociations() {
		$this->loadFixtures('TestPluginEvent', 'User', 'TestPluginComment');
		$TestModel = new TestPluginEvent();

		$result = $TestModel->find('all');
		$expected = array(
			array(
				'TestPluginEvent' => array(
					'id' => 1,
					'user_id' => 1,
					'title' => 'First Plugin Event',
					'body' => 'First Plugin Event Body',
					'published' => 'Y',
				...
				'TestPluginComment' => array(
					array(
						...
						'article_id' => 1,
						'user_id' => 2,
						'comment' => 'First Comment for First Plugin Event',
						'published' => 'Y',
					...
					array(
						...
						'article_id' => 1,
						'user_id' => 4,
						'comment' => 'Second Comment for First Plugin Event',
						'published' => 'Y',
					...
					array(
						...
						'article_id' => 1,
						'user_id' => 1,
						'comment' => 'Third Comment for First Plugin Event',
						'published' => 'Y',
					...
					array(
						...
						'article_id' => 1,
						'user_id' => 1,
						'comment' => 'Fourth Comment for First Plugin Event',
						'published' => 'N',
			...
			array(
				'TestPluginEvent' => array(
					'id' => 2,
					'user_id' => 3,
					'title' => 'Second Plugin Event',
					'body' => 'Second Plugin Event Body',
					'published' => 'Y',
				...
				'TestPluginComment' => array(
					array(
						...
						'article_id' => 2,
						'user_id' => 1,
						'comment' => 'First Comment for Second Plugin Event',
						'published' => 'Y',
					...
					array(
						...
						'article_id' => 2,
						'user_id' => 2,
						'comment' => 'Second Comment for Second Plugin Event',
						'published' => 'Y',
			...
			array(
				'TestPluginEvent' => array(
					'id' => 3,
					'user_id' => 1,
					'title' => 'Third Plugin Event',
					'body' => 'Third Plugin Event Body',
					'published' => 'Y',
 ...
 */
	public function testGetAssociated() {
		...
		foreach ($assocTypes as $type) {
			$this->assertEquals($Event->getAssociated($type), array_keys($Event->{$type}));
		}

		$Event->bindModel(array('hasMany' => array('Category')));
		$this->assertEquals(array('Comment', 'Category'), $Event->getAssociated('hasMany'));

		$results = $Event->getAssociated();
		$results = array_keys($results);
		sort($results);
		$this->assertEquals(array('Category', 'Comment', 'Tag', 'User'), $results);

		$Event->unbindModel(array('hasAndBelongsToMany' => array('Tag')));
		$this->assertEquals(array(), $Event->getAssociated('hasAndBelongsToMany'));

		$result = $Event->getAssociated('Category');
		$expected = array(
			'className' => 'Category',
 ...
 */
	public function testAutoConstructAssociations() {
		$this->loadFixtures('User', 'EventFeatured', 'Featured', 'EventFeaturedsTags');
		...
		$TestModel = new EventFeatured();
		$TestFakeModel = new EventFeatured(array('table' => false));
 ...
 */
	public function testAutoConstructPluginAssociations() {
		...
		$this->assertTrue(isset($Comment->belongsTo['TestPluginEvent']), 'Missing association');
	}
 ...
 */
	public function testColumnTypeFetching() {
		...
		$this->assertEquals('integer', $model->getColumnType('Event.id'));
	}
 ...
 */
	public function testCreation() {
		$this->loadFixtures('Event', 'EventFeaturedsTags', 'User', 'Featured');
		$TestModel = new Test();
		...
		$TestModel = new Event();
		$result = $TestModel->create();
		$expected = array('Event' => array('published' => 'N'));
		$this->assertEquals($expected, $result);
 ...
 */
	public function testHasMethod() {
		$Event = new Event();
		$Event->Behaviors = $this->getMock('BehaviorCollection');

		$Event->Behaviors->expects($this->at(0))
			->method('hasMethod')
			->will($this->returnValue(true));

		$Event->Behaviors->expects($this->at(1))
			->method('hasMethod')
			->will($this->returnValue(false));

		$this->assertTrue($Event->hasMethod('find'));

		$this->assertTrue($Event->hasMethod('pass'));
		$this->assertFalse($Event->hasMethod('fail'));
	}
 ...
 */
	public function testSchemaNoDB() {
		$model = $this->getMock('Event', array('getDataSource'));
