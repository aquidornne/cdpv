 ...
 */
	public function testValidates() {
		...
		$TestModel->validate = array(
			'title' => array(
				'rule' => array('userDefined', 'Event', 'titleDuplicate')
		));
		$data = array('TestValidate' => array('title' => 'My Event Title'));
		$result = $TestModel->create($data);
		$this->assertEquals($data, $result);
		$result = $TestModel->validates();
		$this->assertFalse($result);
		...
		$data = array('TestValidate' => array(
			'title' => 'My Event With a Different Title'
		));
		$result = $TestModel->create($data);
		$this->assertEquals($data, $result);
		$result = $TestModel->validates();
 ...
 */
	public function testStateValidation() {
		...
		$Event->create($data);
		$this->assertFalse($Event->validates());

		$Event->save(null, array('validate' => false));
		$data['Event']['id'] = $Event->id;
		$Event->set($data);
		$this->assertTrue($Event->validates());
 ...
 */
	public function testValidateAssociated() {
		$this->loadFixtures('Comment', 'Attachment', 'Event', 'User');
 ...
 */
	public function testGetMethods() {
		...
		$expected = array_map('strtolower', get_class_methods('Event'));
 ...
 */
	public function testGetModel() {
		...
		$this->assertInstanceOf('Event', $result);
	}
 ...
 */
	public function testValidateCallbacks() {
		$TestModel = $this->getMock('Event', array('beforeValidate', 'afterValidate'));
 ...
 */
	public function testValidateFirstAssociatedWithBeforeValidate() {
		...
		$user = new User();
		$user->bindModel(array('hasMany' => array('CustomEvent')));
		$data = array(
			'User' => array('user' => 'foo', 'password' => 'bar'),
			'CustomEvent' => $articles
		);
 ...
 */
	public function testFirstWithDefaults() {
		...
		unset($result['Event']['updated']);
		$this->assertEquals($expected['Event'], $result['Event']);

		$data = array(
			'Event' => array(
				'id' => 1,
				'title' => 'First Event (modified)'
			),
			'Comment' => array(
				array('comment' => 'Event comment', 'user_id' => 1)
			)
		);
		...
		$expected['Event']['title'] = 'First Event (modified)';
		unset($result['Event']['updated']);
		$this->assertEquals($expected['Event'], $result['Event']);
	}
 ...
 */
	...
	public function testCustomMethodWithEmptyValue() {
		...
		$model = $this->getMock('Event', array('isLegit'));
		$model->validate = array(
			'title' => array(
				'custom' => array(
					'rule' => array('isLegit'),
 ...
 */
	public function testValidateAssociatedAtomicFalseDeepTrueWithErrors() {
		$this->loadFixtures('Comment', 'Event', 'User', 'Attachment');
 ...
 */
	public function testValidateManyAtomicFalseDeepTrueWithErrors() {
		...
		$Event->validateMany($data, array('atomic' => false, 'deep' => true));
