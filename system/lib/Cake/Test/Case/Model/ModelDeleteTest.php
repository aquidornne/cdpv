...
/**
 * testDeleteEventBLinks method
 *
 * @return void
 */
	public function testDeleteEventBLinks() {
		$this->loadFixtures('Event', 'EventsTag', 'Tag', 'User');
		$TestModel = new EventB();

		$result = $TestModel->EventsTag->find('all');
		$expected = array(
			array('EventsTag' => array('article_id' => '1', 'tag_id' => '1')),
			array('EventsTag' => array('article_id' => '1', 'tag_id' => '2')),
			array('EventsTag' => array('article_id' => '2', 'tag_id' => '1')),
			array('EventsTag' => array('article_id' => '2', 'tag_id' => '3'))
			);
		$this->assertEquals($expected, $result);
 ...
 */
	public function testDeleteAll() {
		...
		$result = $TestModel->deleteAll(array('Event.published' => 'N'));
		$this->assertTrue($result);
		...
		$data = array('Event.user_id' => array(2, 3));
		...
		$result = $TestModel->deleteAll(array('Event.user_id' => 999));
 ...
 */
	public function testDeleteAllUnknownColumn() {
		$this->loadFixtures('Event');
		$TestModel = new Event();
		$result = $TestModel->deleteAll(array('Event.non_existent_field' => 999));
 ...
 */
	public function testDeleteAllFailedFind() {
		$this->loadFixtures('Event');
		$TestModel = $this->getMock('Event', array('find'));
 ...
 */
	public function testDeleteAllWithOrderProperty() {
		...
		$TestModel = new Event();
		$TestModel->order = 'Event.published desc';
		$TestModel->unbindModel(array(
			'belongsTo' => array('User'),
			'hasMany' => array('Comment'),
			'hasAndBelongsToMany' => array('Tag')
 ...
 */
	public function testDeleteLinks() {
		$this->loadFixtures('Event', 'EventsTag', 'Tag');
		$TestModel = new Event();

		$result = $TestModel->EventsTag->find('all');
		$expected = array(
			array('EventsTag' => array(
				'article_id' => '1',
				'tag_id' => '1'
			)),
			array('EventsTag' => array(
 ...
 */
	public function testDeleteLinksWithPLuginJoinModel() {
		$this->loadFixtures('Event', 'EventsTag', 'Tag');
		$Event = new Event();
		$Event->unbindModel(array('hasAndBelongsToMany' => array('Tag')), false);
		unset($Event->Tag, $Event->EventTags);
		$Event->bindModel(array('hasAndBelongsToMany' => array(
			'Tag' => array('with' => 'TestPlugin.EventsTag')
		)), false);

		$Event->EventsTag->order = null;
		$this->assertTrue($Event->delete(1));
	}
 ...
 */
	public function testDeleteDependent() {
		$this->loadFixtures('Bidding', 'BiddingMessage', 'Event',
			'EventsTag', 'Comment', 'User', 'Attachment'
		);
		$Bidding = new Bidding();
		...
		$Event = new Event();
		$result = $Event->Comment->find('count', array(
			'conditions' => array('Comment.article_id' => 1)
		));
		$this->assertEquals(4, $result);

		$result = $Event->delete(1, true);
		$this->assertTrue($result);
 ...
 */
	public function testDeleteHabtmPostgresFailure() {
		...
		$Event->hasAndBelongsToMany['Tag']['unique'] = true;
		...
		// Event 1 should have Tag.1 and Tag.2
		$before = $Event->find("all", array(
			"conditions" => array("Event.id" => 1),
		));
		$this->assertEquals(2, count($before[0]['Tag']), 'Tag count for Event.id = 1 is incorrect, should be 2 %s');
		...
		$submittedData = array(
			"Event" => array("id" => 2, 'title' => 'second article'),
			"Tag" => array(
				"Tag" => array(2, 3)
			)
		);
		...
		// MySQL: DELETE `EventsTag` FROM `articles_tags` AS `EventsTag` WHERE `EventsTag`.`article_id` = 2 AND `EventsTag`.`tag_id` IN (1, 3)
		$Event->save($submittedData);

		// Want to make sure Event #1 has Tag #1 and Tag #2 still.
		$after = $Event->find("all", array(
			"conditions" => array("Event.id" => 1),
		));

		// Removing Event #2 from Tag #1 is all that should have happened.
