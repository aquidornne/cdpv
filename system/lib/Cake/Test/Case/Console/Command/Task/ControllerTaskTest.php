...
App::uses('BakeEvent', 'Model');
App::uses('BakeComment', 'Model');
App::uses('BakeTags', 'Model');
$imported = class_exists('BakeEvent') || class_exists('BakeComment') || class_exists('BakeTag');
 ...
 */
	public function setUp() {
		...
		if (!defined('ARTICLE_MODEL_CREATED')) {
			$this->markTestSkipped('Could not run as an Event, Tag or Comment model was already loaded.');
		}
	}
 ...
 */
	public function testListAll() {
		...
		$this->Task->expects($this->at(2))->method('out')->with(' 1. BakeEvents');
		$this->Task->expects($this->at(3))->method('out')->with(' 2. BakeEventsBakeTags');
		$this->Task->expects($this->at(4))->method('out')->with(' 3. BakeComments');
		$this->Task->expects($this->at(5))->method('out')->with(' 4. BakeTags');

		$expected = array('BakeEvents', 'BakeEventsBakeTags', 'BakeComments', 'BakeTags');
 ...
 */
	public function testBakeWithPlugin() {
		...
		$path = APP . 'Plugin' . DS . 'ControllerTest' . DS . 'Controller' . DS . 'EventsController.php';
		...
		$this->Task->expects($this->at(3))->method('createFile')->with(
			$path,
			$this->stringContains('EventsController extends ControllerTestAppController')
		)->will($this->returnValue(true));

		$this->Task->bake('Events', '--actions--', array(), array(), array());

		$this->Task->plugin = 'ControllerTest';
		$path = APP . 'Plugin' . DS . 'ControllerTest' . DS . 'Controller' . DS . 'EventsController.php';
		$result = $this->Task->bake('Events', '--actions--', array(), array(), array());
 ...
 */
	public function testBakeActionsUsingSessions() {
		$result = $this->Task->bakeActions('BakeEvents', null, true);
		$expected = file_get_contents(CAKE . 'Test' . DS . 'bake_compare' . DS . 'Controller' . DS . 'ActionsUsingSessions.ctp');
		$this->assertTextEquals($expected, $result);

		$result = $this->Task->bakeActions('BakeEvents', 'admin_', true);
 ...
 */
	public function testBakeActionsWithNoSessions() {
		$result = $this->Task->bakeActions('BakeEvents', null, false);
 ...
 */
	public function testBakeTest() {
		...
		$this->Task->Test->expects($this->once())->method('bake')->with('Controller', 'BakeEvents');
		$this->Task->bakeTest('BakeEvents');
 ...
 */
	public function testInteractive() {
		...
		$filename = '/my/path/BakeEventsController.php';
		$this->Task->expects($this->once())->method('createFile')->with(
			$filename,
			$this->stringContains('class BakeEventsController')
		);
		$this->Task->execute();
	}
 ...
 */
	public function testExecuteWithController() {
		$this->Task->connection = 'test';
		$this->Task->path = '/my/path/';
		$this->Task->args = array('BakeEvents');
 ...
 */
	public static function nameVariations() {
		return array(
			array('BakeEvents'), array('BakeEvent'), array('bake_article'), array('bake_articles')
		);
	}
