 ...
 */
	public function testImportOptionsSchemaRecords() {
		...
		$result = $this->Task->importOptions('Event');
		$expected = array('schema' => 'Event', 'records' => true);
 ...
 */
	public function testImportRecordsFromDatabaseWithConditionsPoo() {
		...
		$result = $this->Task->bake('Event', false, array(
			'fromTable' => true, 'schema' => 'Event', 'records' => false
		));

		$this->assertContains('class EventFixture extends CakeTestFixture', $result);
		$this->assertContains('public $records', $result);
		$this->assertContains('public $import', $result);
		$this->assertContains("'title' => 'First Event'", $result, 'Missing import data %s');
		$this->assertContains('Second Event', $result, 'Missing import data %s');
		$this->assertContains('Third Event', $result, 'Missing import data %s');
	}
 ...
 */
	public function testImportOptionsAlternateConnection() {
		$this->Task->connection = 'test';
		$result = $this->Task->bake('Event', false, array('schema' => 'Event'));
 ...
 */
	public function testImportRecordsNoEscaping() {
		...
		$Event->updateAll(array('body' => "'Body \"value\"'"));
 ...
 */
	public function testExecuteWithNamedModel() {
		...
		$filename = '/my/path/EventFixture.php';

		$this->Task->expects($this->at(0))->method('createFile')
			->with($filename, $this->stringContains('class EventFixture'));
 ...
 */
	public function testAllWithCountAndRecordsFlags() {
		...
		$this->Task->Model->expects($this->any())->method('listAll')
			->will($this->returnValue(array('Events', 'comments')));

		$filename = '/my/path/EventFixture.php';
		$this->Task->expects($this->at(0))->method('createFile')
			->with($filename, $this->stringContains("'title' => 'Third Event'"));

		$filename = '/my/path/CommentFixture.php';
		$this->Task->expects($this->at(1))->method('createFile')
			->with($filename, $this->stringContains("'comment' => 'First Comment for First Event'"));
 ...
 */
	public function testAllWithSchemaImport() {
		...
		$filename = '/my/path/EventFixture.php';
		$this->Task->expects($this->at(0))->method('createFile')
			->with($filename, $this->stringContains('public $import = array(\'model\' => \'Event\''));
 ...
 */
	public function testExecuteInteractive() {
		...
		$this->Task->Model->expects($this->any())->method('getName')->will($this->returnValue('Event'));
 ...
 */
	public function testBake() {
		...
		$result = $this->Task->bake('Event');
		$this->assertContains('class EventFixture extends CakeTestFixture', $result);
		$this->assertContains('public $fields', $result);
		$this->assertContains('public $records', $result);
		$this->assertNotContains('public $import', $result);

		$result = $this->Task->bake('Event', 'comments');
		$this->assertContains('class EventFixture extends CakeTestFixture', $result);
		$this->assertContains('public $table = \'comments\';', $result);
		$this->assertContains('public $fields = array(', $result);

		$result = $this->Task->bake('Event', 'comments', array('records' => true));
		$this->assertContains("public \$import = array('records' => true, 'connection' => 'test');", $result);
		$this->assertNotContains('public $records', $result);

		$result = $this->Task->bake('Event', 'comments', array('schema' => 'Event'));
		$this->assertContains("public \$import = array('model' => 'Event', 'connection' => 'test');", $result);
		$this->assertNotContains('public $fields', $result);

		$result = $this->Task->bake('Event', 'comments', array('schema' => 'Event', 'records' => true));
		$this->assertContains("public \$import = array('model' => 'Event', 'records' => true, 'connection' => 'test');", $result);
		$this->assertNotContains('public $fields', $result);
		$this->assertNotContains('public $records', $result);
	}
 ...
 */
	public function testRecordGenerationForBinaryAndFloat() {
		...
		$result = $this->Task->bake('Event', 'datatypes');
		$this->assertContains("'float_field' => 1", $result);
		$this->assertContains("'bool' => 1", $result);

		$result = $this->Task->bake('Event', 'binary_tests');
 ...
 */
	public function testGenerateFixtureFile() {
		...
		$this->Task->expects($this->at(0))->method('createFile')
			->with($filename, $this->stringContains('EventFixture'));

		$this->Task->expects($this->at(1))->method('createFile')
			->with($filename, $this->stringContains('<?php'));

		$this->Task->generateFixtureFile('Event', array());
		$this->Task->generateFixtureFile('Event', array());
	}
 ...
 */
	public function testGeneratePluginFixtureFile() {
		...
		$filename = APP . 'Plugin' . DS . 'TestFixture' . DS . 'Test' . DS . 'Fixture' . DS . 'EventFixture.php';

		//fake plugin path
		CakePlugin::load('TestFixture', array('path' => APP . 'Plugin' . DS . 'TestFixture' . DS));
		$this->Task->expects($this->at(0))->method('createFile')
			->with($filename, $this->stringContains('class Event'));
