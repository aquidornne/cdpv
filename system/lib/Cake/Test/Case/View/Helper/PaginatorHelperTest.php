 ...
 */
	public function testHasPrevious() {
		...
		$this->Paginator->request->params['paging']['Event']['prevPage'] = true;
		$this->assertTrue($this->Paginator->hasPrev());
		$this->Paginator->request->params['paging']['Event']['prevPage'] = false;
	}
 ...
 */
	public function testHasNext() {
		...
		$this->Paginator->request->params['paging']['Event']['nextPage'] = false;
		$this->assertFalse($this->Paginator->hasNext());
		$this->Paginator->request->params['paging']['Event']['nextPage'] = true;
	}
 ...
 */
	public function testDisabledLink() {
		$this->Paginator->request->params['paging']['Event']['nextPage'] = false;
		$this->Paginator->request->params['paging']['Event']['page'] = 1;
 ...
 */
	public function testSortLinks() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['sort'] = 'title';
		$result = $this->Paginator->sort('title', array('asc' => 'ascending', 'desc' => 'descending'));
		$expected = array(
		...
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Event.title' => 'desc');
		$this->Paginator->request->params['paging']['Event']['options']['sort'] = null;
		$result = $this->Paginator->sort('title');
		$this->assertRegExp('/\/accounts\/index\/param\/sort:title\/direction:asc" class="desc">Title<\/a>$/', $result);

		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Event.title' => 'asc');
		$this->Paginator->request->params['paging']['Event']['options']['sort'] = null;
		$result = $this->Paginator->sort('title');
 ...
 */
	public function testSortLinkWithVirtualField() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('full_name' => 'asc');

		$result = $this->Paginator->sort('Event.full_name');
		$expected = array(
			'a' => array('href' => '/accounts/index/sort:Event.full_name/direction:desc', 'class' => 'asc'),
			'Event Full Name',
			'/a'
		);
		$this->assertTags($result, $expected);
		...
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('full_name' => 'desc');
		$result = $this->Paginator->sort('Event.full_name');
		$expected = array(
			'a' => array('href' => '/accounts/index/sort:Event.full_name/direction:asc', 'class' => 'desc'),
			'Event Full Name',
			'/a'
		);
		$this->assertTags($result, $expected);
 ...
 */
	public function testSortLinksUsingDotNotation() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Event.title' => 'desc');
		$result = $this->Paginator->sort('Event.title');
		$expected = array(
			'a' => array('href' => '/officespace/accounts/index/sort:Event.title/direction:asc', 'class' => 'desc'),
			'Event Title',
			'/a'
		);
		$this->assertTags($result, $expected);
		...
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Event.title' => 'desc');
		$result = $this->Paginator->sort('Event.title', 'Title');
		$expected = array(
			'a' => array('href' => '/officespace/accounts/index/sort:Event.title/direction:asc', 'class' => 'desc'),
			'Title',
			'/a'
		...
		$result = $this->Paginator->sort('Event.title', 'Title');
		$expected = array(
			'a' => array('href' => '/officespace/accounts/index/sort:Event.title/direction:desc', 'class' => 'asc'),
			'Title',
			'/a'
		);
		$this->assertTags($result, $expected);

		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Account.title' => 'asc');
		$result = $this->Paginator->sort('title');
		$expected = array(
 ...
 */
	public function testSortKey() {
		...
		$result = $this->Paginator->sortKey('Event', array('order' => 'Event.title'));
		$this->assertEquals('Event.title', $result);

		$result = $this->Paginator->sortKey('Event', array('sort' => 'Event.title'));
		$this->assertEquals('Event.title', $result);

		$result = $this->Paginator->sortKey('Event', array('sort' => 'Event'));
 ...
 */
	public function testSortKeyFallbackToParams() {
		$this->Paginator->request->params['paging']['Event']['order'] = 'Event.body';
		$result = $this->Paginator->sortKey();
		$this->assertEquals('Event.body', $result);

		$result = $this->Paginator->sortKey('Event');
		$this->assertEquals('Event.body', $result);

		$this->Paginator->request->params['paging']['Event']['order'] = array(
			'Event.body' => 'DESC'
		);
		$result = $this->Paginator->sortKey();
		$this->assertEquals('Event.body', $result);
 ...
 */
	public function testSortDir() {
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('Event.title' => 'asc');
		$result = $this->Paginator->sortDir();
		$expected = 'asc';
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('title' => 'desc');
		$result = $this->Paginator->sortDir();
		$expected = 'desc';
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$this->Paginator->request->params['paging']['Event']['options']['order'] = array('title' => 'asc');
		$result = $this->Paginator->sortDir();
		$expected = 'asc';
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$this->Paginator->request->params['paging']['Event']['options']['direction'] = 'asc';
		$result = $this->Paginator->sortDir();
		$expected = 'asc';
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$this->Paginator->request->params['paging']['Event']['options']['direction'] = 'desc';
		$result = $this->Paginator->sortDir();
		$expected = 'desc';
		...
		unset($this->Paginator->request->params['paging']['Event']['options']);
		$result = $this->Paginator->sortDir('Event', array('direction' => 'asc'));
		$expected = 'asc';

		$this->assertEquals($expected, $result);

		$result = $this->Paginator->sortDir('Event', array('direction' => 'desc'));
		$expected = 'desc';
 ...
 */
	public function testSortDirFallbackToParams() {
		...
		$result = $this->Paginator->sortDir('Event');
		$this->assertEquals('asc', $result);
 ...
 */
	public function testSortAdminLinks() {
		...
		$result = $this->Paginator->sort('Event.title', 'Title');
		$expected = array(
			'a' => array('href' => '/admin/test/index/param/sort:Event.title/direction:asc'),
			'Title',
			'/a'
		);
		$this->assertTags($result, $expected);
 ...
 */
	public function testUrlGeneration() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['page'] = 2;
		$result = $this->Paginator->url();
		$this->assertEquals('/index/page:2', $result);

		$options = array('order' => array('Event' => 'desc'));
		$result = $this->Paginator->url($options);
		$this->assertEquals('/index/page:2/sort:Event/direction:desc', $result);

		$this->Paginator->request->params['paging']['Event']['options']['page'] = 3;
		$options = array('order' => array('Event.name' => 'desc'));
		$result = $this->Paginator->url($options);
		$this->assertEquals('/index/page:3/sort:Event.name/direction:desc', $result);
	}
 ...
 */
	public function testUrlGenerationWithPrefixes() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['page'] = 2;
		$this->Paginator->request->params['paging']['Event']['page'] = 2;
		$this->Paginator->request->params['paging']['Event']['prevPage'] = true;
		...
		$options = array('controller' => 'posts', 'order' => array('Event.name' => 'desc'));
		$result = $this->Paginator->url($options);
		$expected = '/posts/index/page:2/sort:Event.name/direction:desc';
		$this->assertEquals($expected, $result);
	}
 ...
 */
	public function testOptions() {
		...
		$options = array('paging' => array('Event' => array(
			'order' => 'desc',
			'sort' => 'title'
		)));
 ...
 */
	public function testGenericLinks() {
		...
		$this->Paginator->request->params['paging']['Event']['options']['page'] = 4;
		$result = $this->Paginator->link('Sort by title on page 4', array('sort' => 'Event.title', 'direction' => 'desc'));
		$expected = array(
			'a' => array('href' => '/index/page:4/sort:Event.title/direction:desc'),
 ...
 */
	public function testLastNoOutput() {
		$this->Paginator->request->params['paging']['Event']['page'] = 15;
		$this->Paginator->request->params['paging']['Event']['pageCount'] = 15;
 ...
 */
	public function testFirstFullBaseUrl() {
		...
		$expected = array(
			'<span',
			array('a' => array(
				'href' => FULL_BASE_URL . '/index/sort:Event.title/direction:DESC', 'rel' => 'first'
			)),
			'&lt;&lt; first',
			'/a',
			'/span',
 ...
 */
	public function testLast() {
		...
		$this->Paginator->request->params['paging']['Event']['page'] = 6;
 ...
 */
	public function testHasPage() {
		$result = $this->Paginator->hasPage('Event', 15);
		$this->assertFalse($result);

		$result = $this->Paginator->hasPage('UndefinedModel', 2);
		$this->assertFalse($result);

		$result = $this->Paginator->hasPage('Event', 2);
 ...
 */
	public function testNextLinkUsingDotNotation() {
		...
		$expected = array(
			'span' => array('class' => 'next'),
			'a' => array(
				'href' => '/officespace/accounts/index/page:2/sort:Event.title/direction:asc',
				'rel' => 'next'
			),
			'Next',
			'/a',
 ...
 */
	public function testQuerystringUrlGeneration() {
		$this->Paginator->request->params['paging']['Event']['paramType'] = 'querystring';
 ...
 */
	public function testOptionsConvertKeys() {
		$this->Paginator->options(array(
			...
			'Event' => array('paramType' => 'querystring')
		));
 ...
 */
	public function testCurrent() {
		...
		$this->assertEquals($this->Paginator->request->params['paging']['Event']['page'], $result);
